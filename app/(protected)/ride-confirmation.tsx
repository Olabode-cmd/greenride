 ...
 * coordinates, or null if permission is denied. Used solely to
 * display the user's position on the map — no location data leaves
 * the device.
 */
...
export default function RideConfirmationScreen() {
  ...
  /*
   * Location permission request is a native event listener setup —
   * one of the permitted useEffect use cases per project policy.
   */
  useEffect(() => {
    ...
    <SafeAreaView className="flex-1 bg-bg" edges={["top"]}>
      ...
      >
        ...
        <View style={{ padding: 16, gap: 20 }}>
          ...
          {/*
           * Eco-metric panel — the CO₂ number is the visual hero of this
           * screen per the style guide's "numbers as art" principle.
           */}
          <View
            style={{
              backgroundColor: colors.accentMuted,
      ...
      >
        <Button
          label={paying ? "Processing payment…" : "Confirm & Pay"}
          variant="primary"
          rounded
          haptic
          loading={paying}
...
/*
 * Minimal custom map style — desaturates the base map to keep it from
 * competing with the accent-coloured destination pin.
 */
const mapStyle = [
